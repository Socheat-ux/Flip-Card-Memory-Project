...
function Game({ difficulty, onHome, user }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) { clearInterval(timerRef.current); setLost(true); return 0; }
        return t - 1;
      });
    }, 1000);
  ...
  useEffect(() => {
    if (won) {
      ...
      localStorage.setItem("fc_leaderboard", JSON.stringify([entry, ...prev].slice(0, 50)));
    }
  }, [won]);

  useEffect(() => { if (lost) soundLose(); }, [lost]);
  ...
  return (
    <div className="game">
      ...
      <div className="game-stats">
        ...
        <p className={`timer ${timeLeft <= 10 ? "danger" : ""}`}>{formatTime(timeLeft)}</p>
      </div>
      ...
      <div className="card-grid" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
        {cards.map(card => (
          <Card key={card.id} card={card} isFlipped={isFlipped(card)}
            handleChoice={handleChoice} disabled={disabled} />
        ))}
      </div>
    </div>
  );
